...
これはJavaScriptでDOM操作をするときに把握するべき規定です。`<search>`では`HTMLElement`に規定されているので、現段階では特別にプロパティやメソッドをもつわけではないということがわかります。要素固有のインターフェイスを保つ場合はここが`HTMLSearchElement`のような名前になっていることが多いです。その場合、要素固有のプロパティやメソッドのそれぞれは解説は、要素自体の解説あとに続きます。
...
さて、ここからが本番です。ここから曖昧さが含まれ、解釈の余地することが入ってきます。
